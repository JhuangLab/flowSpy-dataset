...
# Tree plot
plotTree(fspy, color.by = "CD4", show.node.name = T, cex.size = 1) + 
  scale_colour_gradientn(colors = c("#00599F", "#EEEEEE", "#FF3222"))
...
fspy <- buildTree(fspy, dim.type = "tsne", dim.use = 1:2)
# Tree plot
plotTree(fspy, color.by = "CD8", show.node.name = T, cex.size = 1) + 
  scale_colour_gradientn(colors = c("#00599F", "#EEEEEE", "#FF3222"))
...
############# Modify branch id
fspy@meta.data$branch.id[fspy@meta.data$branch.id %in% c(5,2,10)] = "CD4 T cells"
fspy@meta.data$branch.id[fspy@meta.data$branch.id %in% c(7,13)] = "CD8 T cells"
fspy@meta.data$branch.id[fspy@meta.data$branch.id %in% c(1,6,12)] = "Megakaryocytic"
fspy@meta.data$branch.id[fspy@meta.data$branch.id %in% c(3)] = "Dendritic cells"
fspy@meta.data$branch.id[fspy@meta.data$branch.id %in% c(11)] = "B cells"
fspy@meta.data$branch.id[fspy@meta.data$branch.id %in% c(4,8,9,14)] = "Myeloid"
...
# heatmap for Dendritic cells
plot.meta.sub <- plot.meta[which(plot.meta$traj.value.log.DC > 0), ]
plot.meta.sub <- plot.meta.sub[order(plot.meta.sub$pseudotime), ]
pheatmap(t(plot.meta.sub[, markers]), scale  = "row",
         cluster_rows = T, cluster_cols = F, cluster_method = "ward.D",
         color = colorRampPalette(c("blue","blue","white","red","red"))(100),
