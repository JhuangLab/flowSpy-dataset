...
plot2D(fspy, item.use = c("tSNE_1", "tSNE_2"), color.by = "CD38", 
...
plot2D(fspy, item.use = c("UMAP_1", "UMAP_2"), color.by = "CD49f", 
...
fspy <- runPseudotime(fspy, verbose = T, dim.type = "pca", dim.use = 1:4)

fspy <- defLeafCells(fspy, leaf.cells = c(26,25,36), verbose = T)
...
plotViolin(fspy, color.by = "branch.id", order.by = "pseudotime",
           marker = "CD49f", text.angle = 90)
...
plotHeatmap(fspy, downsize = 12000, cluster_rows = T, clustering_method = "ward.D",
...
plotTree(fspy, color.by = "traj.value.log", cex.size = 1, show.node.name = T) + 
 scale_colour_gradientn(colors = c("#EEEEEE","#FF3222","#CC0000","#CC0000"))
...
# run Principal Component Analysis (PCA)
sub.fspy <- runFastPCA(sub.fspy, verbose = T)

# run t-Distributed Stochastic Neighbor Embedding (tSNE)
set.seed(1)
sub.fspy <- runTSNE(sub.fspy, verbose = T)
...
# run Uniform Manifold Approximation and Projection (UMAP)
sub.fspy <- runUMAP(sub.fspy, n_neighbors = 10, verbose = T)

# build minimum spanning tree based on tsne
sub.fspy <- buildTree(sub.fspy, dim.type = "umap", dim.use = 1:2, verbose = T)
...
# Plot marker density
plotMarkerDensity(sub.fspy, adjust = 0.7)

plot2D(sub.fspy, item.use = c("CD43", "CD49f"), color.by = "stage", 
       alpha = 1, main = "tSNE", category = "categorical", show.cluser.id = T)

# Plot 2D tSNE. And cells are colored by cluster id
plot2D(sub.fspy, item.use = c("tSNE_1", "tSNE_2"), color.by = "cluster.id", 
       alpha = 1, main = "tSNE", category = "categorical", show.cluser.id = T)

# Plot 2D tSNE. And cells are colored by stage
plot2D(sub.fspy, item.use = c("tSNE_1", "tSNE_2"), color.by = "stage", 
       alpha = 1, main = "tSNE", category = "categorical", show.cluser.id = T)

# Plot 2D tSNE. And cells are colored by CD45RA markers expression
plot2D(sub.fspy, item.use = c("tSNE_1", "tSNE_2"), color.by = "CD49f", 
               main = "tSNE", category = "numeric")  + 
 scale_colour_gradientn(colors = c("#00599F","#EEEEEE","#FF3222"))

# Plot 2D UMAP. And cells are colored by cluster id
plot2D(sub.fspy, item.use = c("UMAP_1", "UMAP_2"), color.by = "cluster.id", 
       alpha = 1, main = "UMAP", category = "categorical", show.cluser.id = T)

# Plot 2D UMAP. And cells are colored by stage
plot2D(sub.fspy, item.use = c("UMAP_1", "UMAP_2"), color.by = "stage", 
       alpha = 1, main = "UMAP", category = "categorical") +
 scale_color_manual(values = c("#00599F","#009900","#FF9933",
                               "#FF99FF","#7A06A0","#FF3222"))

# Plot 2D UMAP. And cells are colored by stage
plot2D(sub.fspy, item.use = c("DC_2", "DC_3"), color.by = "stage", 
       alpha = 1, main = "UMAP", category = "categorical") +
 scale_color_manual(values = c("#00599F","#009900","#FF9933",
                               "#FF99FF","#7A06A0","#FF3222"))
...
plot3D(sub.fspy, item.use = c("DC_2","DC_1","DC_3"), 
       size = 0.5, color.by = "CD49f", angle = 60, 
       color.theme = c("#00599F","#00599F","#EEEEEE","#FF3222","#FF3222"))

plot3D(sub.fspy, item.use = c("DC_2","DC_1","DC_3"), 
       size = 0.5, color.by = "CD34", angle = 60, 
       color.theme = c("#00599F","#00599F","#EEEEEE","#FF3222","#FF3222"))

plot3D(sub.fspy, item.use = c("DC_2","DC_1","DC_3"), 
       size = 0.5, color.by = "pseudotime", angle = 60, 
       color.theme = c("#F4D31D", "#FF3222","#7A06A0"))

plot3D(sub.fspy, item.use = c("DC_1","DC_2", "DC_3"), color.by = "pseudotime", angle = 120)

# Plot 2D UMAPE. And cells are colored by CD45RA markers expression
plot2D(sub.fspy, item.use = c("UMAP_1", "UMAP_2"), color.by = "CD49f", 
               main = "UMAP CD45RA", category = "numeric")  + 
 scale_colour_gradientn(colors = c("#00599F","#EEEEEE","#FF3222"))
